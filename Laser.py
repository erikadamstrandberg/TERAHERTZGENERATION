...
def TwoC_forward(E,B,E0,xi,phi,PULSELENGTH,PULSESTART,OMEGAPRIM,t0,dt,dz):
    ...
    t = dz*np.arange(START,STOPP,1)
    for i in range(len(El)):
        El[i] = E0*(np.sqrt(1-xi)*np.sin(OMEGAPRIM*t[i])*np.exp(-(t[i]**2/(2*t0**2)))+np.sqrt(xi)*np.sin(2*OMEGAPRIM*t[i]+phi)*np.exp(-4*(t[i]**2/(2*t0**2))))
    t = dz*np.arange(START,STOPP,1)+(dz-dt)/2
    for i in range(len(Bl)):
        Bl[i] = E0*(np.sqrt(1-xi)*np.sin(OMEGAPRIM*t[i])*np.exp(-(t[i]**2/(2*t0**2)))+np.sqrt(xi)*np.sin(2*OMEGAPRIM*t[i]+phi)*np.exp(-4*(t[i]**2/(2*t0**2))))
    E[PULSESTART:PULSESTART+PULSELENGTH] = El
    B[PULSESTART:PULSESTART+PULSELENGTH] = Bl
