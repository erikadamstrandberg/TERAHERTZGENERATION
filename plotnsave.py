...
def plotnsave(y, x = np.array([]), plotargs = '', filename = '', dirname = '', savetext = True, savepic = False, inputfile = '', axis = np.array([]), ylab =  np.array([]), xlab =  np.array([]), xticks =  np.array([]), yticks =  np.array([]), cols = False):
